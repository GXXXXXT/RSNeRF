...
def ray_sample(intersection_outputs, step_size=0.01, fixed=False):
    ...
    sampled_dists = sampled_dists.clamp(min=0.0)     # [N_rays, 111]
    sampled_depth.masked_fill_(sampled_idx.eq(-1), MAX_DEPTH)     # [N_rays, 111]
    sampled_dists.masked_fill_(sampled_idx.eq(-1), 0.0)     # [N_rays, 111]

    samples = {
        "sampled_point_depth": sampled_depth,    # [N_rays, 111]
        "sampled_point_distance": sampled_dists,    # [N_rays, 111]
        "sampled_point_voxel_idx": sampled_idx,    # [N_rays, 111]
    }
    return samples
